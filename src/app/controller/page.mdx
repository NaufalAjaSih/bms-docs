...
- [CRUD Modal vs CRUD Page](#crud-modal-vs-crud-page)
...
public function index(Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) {
                $showUrl = route('route-name.show', $row->id);
                $deleteUrl = route('route-name.destroy', $row->id);
    ...
    return view('admin.{view-folder}.index');
}
```
...
public function show($id)
{
    ...
    return view('admin.{view-folder}.show', compact('data'));
}
```

### Method Create (CRUD Page)
...
public function update(Request $request, $id)
{
    ...
    try {
        ...
        if ($request->hasFile('file_field')) {
            if (!empty($data->file_field) && file_exists(public_path('assets/folder/' . $data->file_field))) {
                unlink(public_path('assets/folder/' . $data->file_field));
            }
...
**Selanjutnya**: [Model Standards](../model) 